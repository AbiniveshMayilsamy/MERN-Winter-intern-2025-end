  ...
  useEffect(() => {
    async function loadAllQuizzes() {
      try {
        ...
        const combined = [...jsonQuizzes, ...savedQuizzes];
        setQuizzes(combined);
        console.log("All Quizzes:", combined);
      } catch (err) {
        ...
        {quizzes.map((quiz) => (
          ...
          >
            ...
            <p>
              ...
              {quiz.questions?.length || quiz.questions || quiz.total_questions}
            </p>
            {quiz.category && (
              <p>
            ...
            <button
              onClick={() => navigate(`/quiz/quiz/${quiz.id}`)}
              style={{
                width: "100%",
                padding: "10px 20px",
